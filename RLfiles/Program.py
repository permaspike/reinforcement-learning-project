...
def Qlearning():
    ...
    statnoRL.updateRL_time(noRL_time)
...
def MClearning():
    ...
    statcontrNoRL.updateRL_time(noRL_time)
    misclassif_stats = [0, 0] #noRL, RL
    for img, label in zip(X_test, y_test):
        no_lr_probabilities_vector = cnn.model.predict(NumpyImg2Tensor(img))
        predictedLabel = np.argmax(no_lr_probabilities_vector)
...
if Q_learning:
    ...
    plot_conf_matrix(dl, networkName, conf_matrix_no_RL, classes, "No RL")
    plot_conf_matrix(dl, networkName, conf_matrix_RL, classes, "Q-Learning")
    num_of_misses.append(statrl.misclassifications[1])
...
if Monte_Carlo:
    ...
    conf_matrix_RL = confusion_matrix(y_test, statrl.predictedLabels)
    plot_conf_matrix(dl, networkName, conf_matrix_no_RL, classes, "No RL")
    plot_conf_matrix(dl, networkName, conf_matrix_RL, classes, "Monte Carlo")
    num_of_misses.append(statrl.misclassifications[1])
    statrl.f1Score = f1_score(y_test, statrl.predictedLabels, average="macro")
    statrl.precision = precision_score(y_test, statrl.predictedLabels, average="macro")
    statrl.recall = recall_score(y_test, statrl.predictedLabels, average="macro")
...
if DeepQ_N:
    ...
    conf_matrix_RL = confusion_matrix(y_test, statrl.predictedLabels)
    plot_conf_matrix(dl, networkName, conf_matrix_no_RL, classes, "No RL")
    plot_conf_matrix(dl, networkName, conf_matrix_RL, classes, "Deep Q Network")
    num_of_misses.append(statrl.misclassifications[1])
    statrl.f1Score = f1_score(y_test, statrl.predictedLabels, average="macro")
    statrl.precision = precision_score(y_test, statrl.predictedLabels, average="macro")
    statrl.recall = recall_score(y_test, statrl.predictedLabels, average="macro")
...
num_of_misses.append(statrl.misclassifications[0])
plot_misclassif(dl,num_of_misses, "qmdn")
